import { getFormated, getStackMap ,itemPoint} from '@/utils/utils'
import { set, get, cloneDeep } from 'utils-lite'
import {markLine,itemStyle as createItemStyle} from "./generateChart";

// default opacity of bar while dim-axis type is 'value'
const VALUE_AXIS_OPACITY = 0.5

function getBarDimAxis (args) {
    const { innerRows, dimAxisName, dimension, axisVisible, dimAxisType, dims,axisLabelRotate } = args
...
function getBarSeries (args) {
    ...
    series = Object.keys(seriesTemp).map((item, index) => {
        ...
        if(!hiddenMarkLine) seriesItem.markLine = markLine(columnsConfig[item])
        if(!hiddenWarnItemStyle) seriesItem.itemStyle = createItemStyle(columnsConfig[item])
...
export const bar = (columns, rows, settings, extra) => {
    ...
    const {
        axisSite = {},
        hiddenMarkLine = false,
        hiddenWarnItemStyle = false,
        columnsConfig = {},
        dimension = [columns[0]],
        stack = {},
        axisVisible = true,
